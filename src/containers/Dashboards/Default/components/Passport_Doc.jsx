...
class PassportDoc extends PureComponent {
  static propTypes = {
  ...
  componentDidMount() {
    ...
    axios.all([axios.get(url, axiosConfig)]).then(axios.spread((branchResponse) => {
      this.setState({
        doc_name: branchResponse.data.data[6].doc_name,
        doc_remain: branchResponse.data.data[6].remains,
        total: branchResponse.data.data[6].count
...
export default withTranslation('common')(PassportDoc);

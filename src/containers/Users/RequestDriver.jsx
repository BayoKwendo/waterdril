...
export class RequestDriver extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            admins: [],
            isLoading: true,
            startDate: moment(new Date()).subtract(60, "days"),
    ...
    getData = (querystring = "", startDate, endDate) => {
        ...
        let url = baseURL + `service_request_driver?id=${USER_ID}&&startDate="${moment(startDate).format('YYYY-MM-DD')}"&endDate="${moment(endDate).format('YYYY-MM-DD')}"`;
        this.setState({
            isLoading: true,
        })
        axios.all([
